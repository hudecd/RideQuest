...
const PROFILE_KEY='ridequest-profile-v1';
let profile=loadProfile();
function loadProfile(){try{const raw=storage.getItem(PROFILE_KEY);return raw?JSON.parse(raw):{name:'Damian',handle:'@damian',avatarData:''}}catch(e){return {name:'Damian',handle:'@damian',avatarData:''}}}
function persistProfile(){storage.setItem(PROFILE_KEY,JSON.stringify(profile));}
let crew=loadCrew();
...
function syncDamianToCrew(){const t=totals();const me=crew.find(x=>x.id==='damian');if(me){me.xp=t.xp;me.rides=rides.length;me.name=profile.name||'Damian';me.handle=profile.handle||'@damian';me.avatar=profile.avatarData?'P':(profile.name||'D').trim().charAt(0).toUpperCase()||'D';persistCrew()}}
...
function nav(screen){const current=document.querySelector('.screen.active');if(current?.id===`screen-${screen}`)return;document.querySelectorAll('.screen').forEach(s=>s.classList.remove('active'));const next=document.getElementById(`screen-${screen}`);next.classList.remove('active');void next.offsetWidth;next.classList.add('active');document.querySelectorAll('.nav button').forEach(b=>b.classList.toggle('active',b.dataset.go===screen));if(screen==='dashboard')renderDashboard();if(screen==='history')renderHistory();if(screen==='community')renderCommunity();if(screen==='stats')renderStats();if(screen==='profile')renderProfile();window.scrollTo({top:0,behavior:'smooth'});}
...
function renderDashboard(){
  ...
  document.querySelectorAll('#dashboardLevelMini').forEach(el=>el.textContent=level.name);
  const dav=document.getElementById('dashboardAvatar');if(dav){dav.innerHTML=profile.avatarData?`<img src="${profile.avatarData}" alt="Profilovka">`:esc((profile.name||'Damian').trim().charAt(0).toUpperCase())}
...
const BADGES=[
 {id:'first',icon:'⚡',name:'First Ride',desc:'Prvá uložená jazda',ok:t=>t.rides>=1},
 {id:'five',icon:'🧭',name:'Trail Habit',desc:'5 jázd',ok:t=>t.rides>=5},
 {id:'ten',icon:'🏔️',name:'Mountain Mode',desc:'10 jázd',ok:t=>t.rides>=10},
 {id:'xp100',icon:'✦',name:'Vector Core',desc:'100 XP',ok:t=>t.xp>=100},
 {id:'xp500',icon:'🔷',name:'Gravity Pulse',desc:'500 XP',ok:t=>t.xp>=500},
 {id:'xp1000',icon:'💠',name:'Apex Signal',desc:'1 000 XP',ok:t=>t.xp>=1000},
 {id:'km10',icon:'🛞',name:'First 10K',desc:'10 km spolu',ok:t=>t.distance>=10},
 {id:'km50',icon:'🌌',name:'Orbit 50',desc:'50 km spolu',ok:t=>t.distance>=50},
 {id:'time300',icon:'⏱️',name:'Five Hour Rider',desc:'5 h v sedle',ok:t=>t.time>=300},
];
function renderProfile(){
 const t=totals(),{level}=levelFor(t.xp);syncDamianToCrew();
 const av=document.getElementById('profileAvatar');if(av)av.innerHTML=profile.avatarData?`<img src="${profile.avatarData}" alt="Profilovka">`:esc((profile.name||'Damian').trim().charAt(0).toUpperCase());
 const nameView=document.getElementById('profileNameView');if(nameView)nameView.textContent=profile.name||'Damian';
 const handleView=document.getElementById('profileHandleView');if(handleView)handleView.textContent=profile.handle||'@damian';
 const pill=document.getElementById('profileLevelPill');if(pill)pill.textContent=`◈ ${level.name} · ${t.xp} XP`;
 const pn=document.getElementById('profileName');if(pn&&document.activeElement!==pn)pn.value=profile.name||'Damian';
 const ph=document.getElementById('profileHandle');if(ph&&document.activeElement!==ph)ph.value=profile.handle||'@damian';
 const mini=document.getElementById('profileMiniStats');if(mini)mini.innerHTML=[['XP',t.xp],['Jazdy',rides.length],['Kilometre',`${t.distance.toFixed(1)} km`]].map(x=>`<div class="profile-mini"><b>${x[1]}</b><span>${x[0]}</span></div>`).join('');
 const grid=document.getElementById('badgeGrid'),unlocked=BADGES.filter(b=>b.ok(t)).length;if(grid)grid.innerHTML=BADGES.map(b=>`<div class="badge-item ${b.ok(t)?'unlocked':'locked'}"><div class="badge-icon">${b.icon}</div><div class="badge-name">${b.name}</div><div class="badge-desc">${b.desc}</div></div>`).join('');
 const bc=document.getElementById('badgeCount');if(bc)bc.textContent=`${unlocked} / ${BADGES.length} odomknutých`;
}
const avatarInput=document.getElementById('avatarInput');
if(avatarInput)avatarInput.addEventListener('change',()=>{const file=avatarInput.files?.[0];if(!file)return;if(!file.type.startsWith('image/'))return;const reader=new FileReader();reader.onload=()=>{profile.avatarData=String(reader.result||'');persistProfile();renderProfile();renderDashboard();};reader.readAsDataURL(file)});
const saveProfile=document.getElementById('saveProfile');
if(saveProfile)saveProfile.addEventListener('click',()=>{const n=document.getElementById('profileName').value.trim(),h=document.getElementById('profileHandle').value.trim();profile.name=n||'Damian';profile.handle=h? (h.startsWith('@')?h:`@${h}`):'@damian';persistProfile();syncDamianToCrew();renderProfile();renderDashboard();showToast('Profil uložený');});
...
resetForm();renderDashboard();renderCommunity();renderProfile();
